...
def test_post_device_information(device_group):
    ...
    device_1 = Device.objects.get(external_id="ST2010-2758")
    device_state_1 = (
        DeviceState.objects.filter(device__id=device_1.id)
        .order_by("-created_at")
        .first()
    )
    ...
    device_2 = Device.objects.get(external_id="ST2010-2762")
    device_state_2 = (
        DeviceState.objects.filter(device__id=device_2.id)
        .order_by("-created_at")
        .first()
    )
    ...
    device_state_3 = (
        DeviceState.objects.filter(device__id=device_3.id)
        .order_by("-created_at")
        .first()
    )
    ...
    device_state_4 = (
        DeviceState.objects.filter(device__id=device_4.id)
        .order_by("-created_at")
        .first()
    )
...
def device_group(db, django_user_model):
    ...
    organization = Organization.objects.create(
        name="WPS", description="An Organization for testing."
    )

    outbound_type = OutboundIntegrationType.objects.create(
        name="EarthRanger", description="ER"
    )

    outbound_config = OutboundIntegrationConfiguration.objects.create(
        type_id=outbound_type.id, owner_id=organization.id
    )
    ...
    device_group = DeviceGroup.objects.create(
        owner_id=outbound_config.owner.id, name=name
    )
    ...
    inbound_config = InboundIntegrationConfiguration.objects.create(
        type_id=inbound_type.id,
