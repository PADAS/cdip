...
def test_get_organization_detail_organization_member_viewer(
    client, organization_member_user, setup_data
):
    ...
    response = client.get(
        reverse("organizations_detail", kwargs={"module_id": org1.id}),
    ...
    response = client.get(
        reverse("organizations_detail", kwargs={"module_id": org2.id}),
        HTTP_X_USERINFO=organization_member_user.user_info,
    )
...
def test_add_organizations_list_global_admin(client, global_admin_user):
    ...
    # Get organizations list
    response = client.get(
        reverse("organizations_add"), HTTP_X_USERINFO=global_admin_user.user_info
    )
...
def test_update_organizations_list_global_admin(client, global_admin_user, setup_data):
    ...
    # Get organizations list
    response = client.get(
        reverse("organizations_update", kwargs={"organization_id": org1.id}),
