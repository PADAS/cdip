...
def test_get_integration_type_list(mock_get_user_perms, mock_decode_token, client, django_user_model):

        mock_get_user_perms.return_value = ['read:inboundintegrationtype', 'core.admin']
        mock_decode_token.return_value = {'sub': '821894f4-755b-4677-8b8f-0e6c557469c5'}
        password = django_user_model.objects.make_random_password()
        user_const = dict(last_name='Owen', first_name='Harry')
        user = django_user_model.objects.create_superuser(
            '821894f4-755b-4677-8b8f-0e6c557469c5', 'harry.owen@vulcan.com', password,
            **user_const)
        ...
        response = client.get(reverse("inboundintegrationtype_list"), HTTP_AUTHORIZATION='Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyLCJraWQiOiJhc2RmYXNkZiJ9.MCbn2FuYX5mpVE20t6J7JmqFCWRTIi21wM7FOjm8ZSo')
        print(response.json())
...
# @pytest.fixture
# def user_oidc_profile(db):
#     from django_keycloak.models import OpenIdConnectProfile, Realm, Server
#     return OpenIdConnectProfile(realm=Realm(server=Server(url='http://tempuri.org')), sub="foo")
